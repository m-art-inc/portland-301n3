(function(module) {
    ...
    Article.createTable = function(callback) {
        webDB.execute(
            ...
            'CREATE TABLE IF NOT EXISTS articlesDB (' +
            ...
            'body TEXT NOT NULL);',
            function(result) {
                console.log('Successfully set up the articlesDB table', result);
                if (callback) {
                    callback();
                }
            }
    ...
    Article.truncateTable = function(callback) {
        webDB.execute(
            'DELETE FROM articlesDB', // <----finish the command here, inside the quotes.
            callback
        );
    };
    ...
    Article.prototype.insertRecord = function(callback) {
        webDB.execute(
            [{
                'sql': 'INSERT INTO articlesDB (title, category, author, authorUrl, publishedOn, body) VALUES (?, ?, ?, ?, ?, ?)',
    ...
    Article.prototype.deleteRecord = function(callback) {
        webDB.execute(
            [{
              sql: 'DELETE FROM articlesDB WHERE id=?',
              data: [this.id],
            }],
            callback
        );
    ...
    // TODO/DONE: Update an article instance, overwriting it's properties into the corresponding record in the database:
    Article.prototype.updateRecord = function(callback) {
        webDB.execute(
            [{
              sql: 'UPDATE articlesDB SET (title, category, author, authorUrl, publishedOn, body) VALUES (?, ?, ?, ?, ?, ?) WHERE id=?',
    ...
    Article.fetchAll = function(next) {
        webDB.execute('SELECT * FROM articlesDB', function(rows) { // TODO/DONE: fill these quotes to 'select' our table.
            if (rows.length) {
                ...
                $.getJSON('data/hackerIpsum.json', function(rawData) {
                    ...
                    webDB.execute('SELECT * FROM articlesDB', function(rows) { // TODO/DONE: select our now full table
                        // TODO/DONE: Now, 1st - instanitate those rows with the .loadAll function,
                        // and 2nd - pass control to the view by calling whichever function argument was passed in to fetchAll.
